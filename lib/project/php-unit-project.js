...
import {removeLeadingChars, readXmlFile, createXmlDocument, isFile} from '../util/php-unit-utils'
...
export default class PhpUnitProject
{
    ...
    * getTestClassPaths (dir = null, filter = null) {
    ...
    runTest (options = {}) {
        ...
        let extensions = []
        if (((this.debugState.toggle && this.debugState.enabled) || this.coverageState.enabled) && this.phpExtensions.length) {
            for (const ext of this.phpExtensions) {
                extensions.push(`-dzend_extension="${ext}"`)
            }
        }
    ...
    /**
     * Validates the extensions point to real files
     *
     * @return {Promise}
     */
    checkExtensionsValidity () {
        const extensions = this.config.get('phpExtensions').split(':').filter(v => !!v)
        const batch = []

        for (const ext of extensions) {
            batch.push(async () => {
                const exists = await isFile(ext)

                if (!exists) {
                    throw new Error(`The PHP extension '${ext} does not exist.'`)
                }
    ...
    async checkDebugAvailability () {
        await Promise.all([
            this.checkExtensionsValidity(),
            this.checkGlobalDebug()
        ])
        ...
        if (!this.debugState.global) {
            for (const ext of this.phpExtensions) {
                if (ext.toLowerCase().includes('xdebug')) {
