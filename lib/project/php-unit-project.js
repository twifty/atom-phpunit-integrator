...
export default class PhpUnitProject
{
	...
	/**
	 * Configures the active test suite name
	 *
	 * @param {String} suiteName - A valid suite name
	 */
	setActiveTestSuiteName (suiteName) {
	 ...
	 * @return {String}
	 */
	getActiveTestSuiteName () {
		return this.activeTestSuite
	}
	...
	killTest () {
		// TODO Implement a method of killing current and queued tests.
	}
	...
	async execute (command, options) {
		return new Promise((resolve, reject) => {
			...
			const runtime = spawn(command[0], command.splice(1), {cwd: path})
