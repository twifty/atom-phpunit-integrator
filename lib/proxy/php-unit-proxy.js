...
export default class PhpUnitProxy
{
	...
	getScopeForPoint (editor, point) {
		...
		if (this.classRanges.has(editor)) {
			...
			if (mappedClass) {
				return new PhpUnitScope(mappedClass, mappedMethod)
			}
		}
	...
	async getClassScopesForFile (file) {
		...
		for (const className of classList) {
			resolvers.push((async () => {
				...
				return new PhpUnitScope(classMap)
			})())
		}
	...
	// getMethodScopesForClass (className) {
	//
	// }

	/**
	 * Checks if a point is within a method body
	 *
	 * @param  {TextEditor}        editor - The text editor to examine
	 * @param  {Point}             point  - The editor buffer position
	 *
	 * @return {Boolean}
	 */
	// pointInMethodRange (editor, point) {
	...
	//
	// /**
	//  * Checks if a point is within a class body
	//  *
	//  * @param  {TextEditor}        editor - The text editor to examine
	//  * @param  {Point}             point  - The editor buffer position
	//  *
	//  * @return {Boolean}
	//  */
	// pointInClassRange (editor, point) {
	// 	if (this.classRanges.has(editor)) {
	...
	async validateScope (scope) {
		...
		if (scope.method && (typeof scope.method.name !== 'string' || '' === scope.method.name)) {
			throw new Error(`Scope contains an invalid 'method.name' property (${scope.method.name})`)
		}
		...
		try {
			...
			if (error.response && error.response.error && error.response.error.code === -32001) {
				atom.notifications.addError("File found but class missing", {
					detail: `The source file '${scope.path}' exists, but the database ` + 'could not find any classes within the file.\n\n' + 'Either remove the empty file or reindex the project.',
					dismissable: true
				})
			} else {
				throw error
		...
		if (!(scope instanceof PhpUnitScope)) {
			scope = new PhpUnitScope(scope, scope.method)
		}
	 ...
	 *
	 * @todo this should be moved to project/manager/tester
	 * @private
