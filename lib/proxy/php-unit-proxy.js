...
export default class PhpUnitProxy
{
    ...
    getScopeForCursor (editor) {
        if (editor) {
            ...
            const error = this.service.getParseError(editor.getPath())

            if (!scope && error) {
                atom.notifications.addError(`Error parsing file ${editor.getPath()}`, {
    ...
    async createRangesForEditor (editor) {
        ...
        const classInfos = await this.service.getClassInfosForFile(path, editor.getText(), true)
