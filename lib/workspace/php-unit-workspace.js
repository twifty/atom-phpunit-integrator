...
export default class PhpUnitWorkspace
{
    ...
    registerCommandHandlers () {
        ...
        const validateAndRun = async (isMethod) => {
            ...
            if (!scope) {
                // console.error('registerCommandHandlers::validate Failed to find a scope')
                return
            }
            ...
            try {
                ...
                if (isMethod) {
                    await this.projectTester.runTestMethod({scope})
                } else {
                    await this.projectTester.runTestClass({scope})
                }
            } catch (error) {
                if ('cancelled' !== error) {
                    throw error
