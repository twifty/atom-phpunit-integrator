...
export default class PhpUnitIntegratorView
{
	 ...
	 *
	 * @constructor
	 * @param {PhpUnitTestRunner} testRunner - The project test runner
	 * @param {PhpUnitConfig}     config     - Project configuration
	 * @param {Array}             [state]    - The previous result of @see {@link getState}
	 */
	constructor (testRunner, config, state = {}) {
		this.testRunner = testRunner
		this.projectManager = testRunner.getProjectManager()
	...
	update (props) {
		...
		if (props.activeProject && props.activeProject !== this.selectedProject) {
			...
			tasks.push(this.updateProjectOptionList())
			tasks.push(this.updateSuiteOptionList())
		}
		...
		switch (props.state) {
			case 'test-begin':
				...
				if (this.refs.term) {
					tasks.push(this.refs.term.clear())
				}
				break;
	...
	async clear () {
		...
		try {
			...
			this.buttonOptions.clearAll.disabled = true
	...
	buildSuiteOptionList () {
		const project = this.projectManager.getProject(this.selectedProject)
		const active = project.getActiveTestSuiteNames()
	...
	disableButtons(disable) {
		return this.scheduleUpdate(() => {
			...
			this.buttonOptions.clearAll.disabled = disable
	...
	onSelectProject (projectPath) {
		...
		return this.updateSuiteOptionList()
	}
	...
	onSelectSuites (suiteNames) {
		...
		this.testRunner.setActiveTestSuiteNames(project, suiteNames)

		this.buttonOptions.runSuite.disabled = 0 === suiteNames.length
	...
	onToggleCoverage () {
		...
		this.testRunner.enableCodeCoverage(project, coverage)
	...
	async onClickButton (event) {
		switch (event.button) {
			...
			case 'filterFailure':
			case 'filterSkipped':
				return this.refs.report.applyFilterStates(event.states)
		}
	}
		...
		if (this.state.hasTerminal) {
			...
			terminalView.push(
				<EtchFlexElement key="term" flex={2}>
					<EtchTerminal enableInput={false} onPreRenderLine={this.onPreRenderOutputLine.bind(this)} className="php-unit-output-view" ref="term"/>
				</EtchFlexElement>
			)
		}
				...
				<div className="php-unit-pane-content">
					...
					<EtchFlexContainer ref="container" state={this.state && this.state.container} orientation={ this.state.orientation }>
						...
						<EtchFlexElement flex={1}>
							<PhpUnitReportView onDidSelect={this.onSelectCase.bind(this)} ref="report"/>
						</EtchFlexElement>
