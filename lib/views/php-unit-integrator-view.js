...
export default class PhpUnitIntegratorView
{
	 ...
	 */
	beginTestListeners () {
		this.disposables.add(this.projectTester.onWillBeginBatchTest(({project, count}) => {
			this.pendingTests = count

			// TODO The projects reports should be merged at the source
		...
		this.disposables.add(this.projectTester.onDidBeginTest(({project}) => {
			if (project.getRoot() !== this.selectedProject) {
		...
		return (<div className="php-unit-integrator">
			...
			<EtchFlexContainer ref="container" state={this.state && this.state.container} orientation={ this.orientation }>
				...
				<EtchFlexElement flex={1}>
					<PhpUnitReportView onRunSuite={this.runSuite.bind(this)} onRunSelection={this.runSuite.bind(this)} onClearAll={this.clearAll.bind(this)} ref="report"/>
				</EtchFlexElement>
