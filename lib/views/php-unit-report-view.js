...
export default class PhpUnitReportView
{
	...
	createErrorItems (errors) {
		return errors.map(errorReport => {
			...
			let trace = errorReport.getTrace()
				...
				for (const item of trace) {
					lines.push(
						<div>
							<span
								className="file-link"
								onClick={ openInAtom.bind(null, item.getFilePath(), item.getFilePosition()) }
							>
								{ item.getDisplay() }
							</span>
						</div>
		...
		return (
			...
			>
				<span>
					...
					<span className={'test-state php-test-' + report.getState()}>{state}</span>
				</span>
				{ errors }
				{ children || null }
			</EtchTreeNode>
	...
	render () {
		...
		['passed', 'error', 'warning', 'failure', 'skipped'].forEach((state) => {
			if (0 <= seenStates.indexOf(state)) {
				filters.push(<li>
					<button ref={'btn-' + state} onClick={this.onFilterList.bind(this, state)} className={'icon fa-lg ' + icons[state]}/>
				</li>)
			}
		})
		...
		return (<div className="php-unit-report-view">
			<ul className="php-unit-report-nav">
				<li>
					<button onClick={this.onClickButton.bind(this, 'run-suite')} className={'icon fa-lg fa fa-run-all'}/>
				</li>
				<li>
					<button { ...isRunButtonDisabled } ref="runSelected" onClick={this.onClickButton.bind(this, 'run-selected')} className={'icon fa-lg fa fa-run-select'}/>
				</li>
				<li>
					<button onClick={this.onClickButton.bind(this, 'clear-all')} className={'icon fa-lg fa fa-clear-all'} { ...isClearButtonDisabled }/>
				</li>
				{filters}
			</ul>
