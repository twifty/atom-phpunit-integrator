...
export default class PhpUnitReportView
{
	...
	constructor (options) {
		...
		this.filteredStates = createFilterStates(options.filterStates)
	 ...
	 */
	update (properties) {
		if (null != properties.pendingReport) {
			this.pendingReport = properties.pendingReport
		}
		...
		this.filteredStates = createFilterStates(properties.filterStates)
	...
	createListItem (report, ref, children) {
		...
		if (report.isCaseReport()) {
			...
			if (state in this.filteredStates) {
				attributes.disabled = true
			}
		} else {
			let count = 0
			for (const child of children) {
				if (child.props.attributes && child.props.attributes.disabled) {
					count++
				}
			}
		...
		//
		// 		this.clickHandler(this.refs[ref].element)
		// 	}
		// }
