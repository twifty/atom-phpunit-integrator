...
export default class PhpUnitReportView
{
   ...
   */
  createListItem (testCase, ref, children) {
    ...
    const handleDoubleClick = () => {
      openInAtom(testCase.file, testCase.line)
    }
    ...
    if (this.meta) {
      ...
      this.meta.forEach(testSuite => {
        const nodes = testSuite.cases.map((testCase) => {
          ...
          return this.createListItem(testCase, 'case-' + ++cases)
        })

        children.push(this.createListItem(testSuite, 'suite-' + ++suites, nodes))
      })
    }
