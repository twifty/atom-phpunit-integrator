...
export default class PhpUnitTestReport
{
   ...
   *
   * @return {Promise<Object>} - Resolves with the test report
   */
  getTestCases () {
    if (!this.parser) {
  ...
  getTestSuites () {
    return this.getTestCases().then(testCases => {
      ...
      testCases.forEach(testCase => {
        ...
        // const suiteFile = testCase.file || null
   ...
   *
   * @return {Object}        - The converted results
   */
  parseTestSuite (suite) {
    ...
    suites.forEach(_suite => {
      meta = meta.concat(this.parseTestSuite(_suite))
    })

    cases.forEach(_case => {
      meta.push(this.parseTestCase(_case))
    })
