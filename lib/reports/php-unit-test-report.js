	...
	buildTestSuiteReports () {
		if (!this.testSuites) {
			...
			caseReports.forEach(caseReport => {
				...
				const suiteReport = this.testSuites[suiteName] || new SuiteReport(suiteName)
		...
		for (const type of ['error', 'warning', 'failure', 'skipped']) {
			...
			if (children.length === 1) {
				...
				for (index; index < lines.length; index++) {
					...
					if (inTrace || TRACE_EXPRESSION.test(line)) {
						inTrace = true
						let [path, num] = line.split(':')
						trace.push(new ErrorTrace({
							display: line,
							file: path,
							line: num
						}))
