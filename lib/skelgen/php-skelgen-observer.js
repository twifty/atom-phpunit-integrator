...
/*
 * This is more of an editor manager. It uses the skelgen factory to find a test
 * file and optionaly populate a new file with content. The proxy is used to find
 * a scope for the context menu and to convert a source scope into a target scope.
 *
 * TODO Each scope should reference its owning project, the factory should then
 * come from each project.
 */
	...
	async resolveTestSuite (source, options = {}) {
		...
		if (!target.getClassRange()) {
			if (options.shouldCreate) {
				try {
					return await this.promptToCreate(source, target, options)
				} catch (error) {
					if ('cancelled' !== error) {
						throw error
					}
