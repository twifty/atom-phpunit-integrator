...
export default class PhpUnitTestRunner
{
	 ...
	 */
	async enableCodeCoverage (toggle) {
		toggle = !!toggle

		for (const project of this.projectManager.getProjects()) {
			project.toggleCodeCoverage(toggle)
		}
	 ...
	 * @return {Promise}                  - Resolve when all listeners have resolved
	 */
	async setSelectedSuiteNames (project, suiteNames) {
		const resolved = await this.resolveOptions({project})

		resolved.project.setSelectedSuiteNames(suiteNames)
	...
	async runTestFile ({project = null, path = null} = {}) {
		try {
			...
			for (let scope of scopes) {
				if (!scope.isTestFile()) {
					scope = await this.proxy.resolveTestSuite(scope)
	...
	async runTestClass ({project = null, scope = null, className = null} = {}) {
		try {
			...
			if (this.config.get('gotoTest')) {
				await openInAtom(resolved.scope.getPath(), resolved.scope.getRange().start)
			}
	...
	async runTestMethod ({project = null, scope = null, className = null, methodName = null} = {}) {
		try {
			...
			if (!resolved.scope.isTestFile()) {
				resolved.scope = await this.proxy.resolveTestCase(resolved.scope)
			}
			...
			if (this.config.get('gotoTest')) {
				await openInAtom(resolved.scope.getPath(), resolved.scope.getMethodRange().start)
			}
