...
export default class PhpUnitTestRunner
{
	 ...
	 * @return {Promise}                  - Resolve when all listeners have resolved
	 */
	async setActiveTestSuiteNames (project, suiteNames) {
		const resolved = await this.resolveOptions({project})

		// if (suiteName !== resolved.project.getActiveTestSuiteName()) {
			resolved.project.setActiveTestSuiteNames(suiteNames)

			return this.emitter.emitAsync('active-test-suites-changed', {
				project: resolved.project,
				suiteNames
			})
		// }
	 ...
	 * @return {Disposable}
	 */
	onActiveTestSuitesChanged (cb) {
		return this.emitter.on('active-test-suites-changed', cb)
	}
	...
	async runTestSuite ({project = null, suiteNames = null, filter = {}} = {}) {
		try {
			...
			if (null === suiteNames) {
				suiteNames = resolved.project.getActiveTestSuiteNames()
